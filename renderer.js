...
function applyEngineUI() {
  ...
  claudeBtn.innerHTML = icoSvg('play') + (currentEngine === 'glm' ? ' เริ่ม opencode' : ' เริ่ม claude');
}
function updateHint() {
  const h = document.getElementById('hint');
  if (!h) return;
  if (currentEngine === 'glm') {
    h.innerHTML = 'engine ปัจจุบัน: <code>GLM (OpenCode)</code> · คลุมคำในโน้ตแล้วกดปุ่ม action เพื่อให้ AI แก้โน้ตให้ (ไม่ต้อง login)';
  } else {
    h.innerHTML = 'engine ปัจจุบัน: <code>Claude</code> · ครั้งแรกกด “เริ่ม claude” เพื่อ login';
  }
}
...
function persistSessions(){
  try {
    ...
    localStorage.setItem('aiSessions', JSON.stringify({ sessions: slim, activeId }));
  } catch (_) {}
}
...
function loadSessions(){
  let saved = null;
  try { saved = JSON.parse(localStorage.getItem('aiSessions') || 'null'); } catch (_) {}
...
function renderTabs(){
  ...
  sessions.forEach((s) => {
    ...
    t.className = 'stab' + (s.id === activeId ? ' active' : '') + (s.engine === 'glm' ? ' g' : ' c');
    ...
    t.appendChild(ic); t.appendChild(nm); t.appendChild(eng);
    if (s.running) { const d = document.createElement('span'); d.className = 'rundot'; t.appendChild(d); }
    t.onclick = () => switchSession(s.id);
    bar.appendChild(t);
  });
  const add = document.createElement('div'); add.className = 'stab add'; add.textContent = '＋'; add.title = 'session ใหม่';
...
function renderHead(){
  ...
  head.appendChild(nm);
  const scope = document.createElement('select'); scope.className = 'sh-scope'; scope.title = 'ขอบเขตบริบท';
  ['note','vault','free'].forEach((v) => { const o = document.createElement('option'); o.value = v; o.textContent = SCOPE_LABEL[v]; if (s.scope === v) o.selected = true; scope.appendChild(o); });
  ...
  const dots = document.createElement('button'); dots.className = 'sh-dots'; dots.textContent = '⋯'; dots.title = 'จัดการ session';
...
function renderChat(){
  ...
  s.messages.forEach((m, i) => {
    ...
    else { b.textContent = m.text; if (running) liveBubble = b; }
    wrap.appendChild(b); box.appendChild(wrap);
  });
  chatScroll();
}
function renderSessions(){ renderTabs(); renderHead(); renderChat(); }
function switchSession(id){ if (id === activeId) return; activeId = id; syncEngineFromSession(); persistSessions(); renderSessions(); }
function addSession(){
...
function openSessionMenu(anchor, s){
  ...
  const items = [ ['เปลี่ยนชื่อ', () => startRename(s)], ['ล้างแชต', () => { s.messages = []; persistSessions(); renderChat(); }] ];
  if (sessions.length > 1) items.push(['ลบ session', () => deleteSession(s)]);
...
window.api.onEngineOutput((payload) => {
  ...
  if (shown) { last.text = shown; if (runId === activeId && liveBubble) { liveBubble.textContent = shown; chatScroll(); } }
});
window.api.onEngineDone((payload) => { if (payload && typeof payload.code !== 'undefined') term.write('\r\n\x1b[32m[done] เสร็จ (exit ' + payload.code + ')\x1b[0m\r\n'); });
window.api.onEngineDone((payload) => {
  const runId = payload && payload.runId; if (!runId) return;
  const s = sessionById(runId); if (!s) return;
  const last = s.messages[s.messages.length - 1];
  if (last && last.role === 'ai') { const shown = cleanChatText(last._acc || '').trim(); last.text = shown || '(เสร็จ · ไม่มีข้อความตอบกลับ)'; delete last._acc; }
  s.running = false;
...
// ---------- Sidebar: folder tree ----------
const collapsedFolders = new Set((() => { try { return JSON.parse(localStorage.getItem('collapsedFolders') || '[]'); } catch (_) { return []; } })());
function persistCollapsed(){ try { localStorage.setItem('collapsedFolders', JSON.stringify([...collapsedFolders])); } catch (_) {} }
...
async function moveNote(from, folderPath){
  ...
  if (r && r.error === 'exists') { alert('มีโน้ตชื่อนี้อยู่ในโฟลเดอร์นั้นแล้ว'); return; }
...
function makeFolderRow(node, depth){
  ...
  const ic = document.createElement('span'); ic.className = 'folder-ic'; ic.innerHTML = icoSvg('folder', 'sm');
  ...
  row.onclick = () => { if (collapsed) collapsedFolders.delete(node.path); else collapsedFolders.add(node.path); persistCollapsed(); renderTree(); };
  row.oncontextmenu = (e) => { e.preventDefault(); openFolderMenu(row, node); };
  row.addEventListener('dragover', (e) => { if (noteDragSrc || pdfDragSrc) { e.preventDefault(); row.classList.add('drop-hi'); } });
  row.addEventListener('dragleave', () => row.classList.remove('drop-hi'));
  row.addEventListener('drop', (e) => { e.preventDefault(); row.classList.remove('drop-hi'); if (pdfDragSrc) movePdf(pdfDragSrc, node.path); else if (noteDragSrc) moveNote(noteDragSrc, node.path); });
  return row;
}
function renderFolderNode(node, container, depth){
  ...
  pdfs.forEach((p) => {
    const comp = (typeof companionNoteName === 'function') ? companionNoteName(p) : p.replace(/\.pdf$/i, '') + ' · โน้ต.md';
    if (node.notes.includes(comp)) { companionOf[p] = comp; companionSet.add(comp); }
  });
  ...
  pdfs.forEach((p) => {
    ...
    if (companionOf[p]) container.appendChild(makeNoteItem(companionOf[p], depth + 1, { companion: true, icon: 'bookmark', label: 'โน้ตของเล่มนี้' }));
  });
}
...
noteList.addEventListener('dragover', (e) => { if ((noteDragSrc || pdfDragSrc) && e.target === noteList) { e.preventDefault(); noteList.classList.add('drop-hi-root'); } });
noteList.addEventListener('dragleave', (e) => { if (e.target === noteList) noteList.classList.remove('drop-hi-root'); });
noteList.addEventListener('drop', (e) => { if ((noteDragSrc || pdfDragSrc) && e.target === noteList) { e.preventDefault(); noteList.classList.remove('drop-hi-root'); if (pdfDragSrc) movePdf(pdfDragSrc, ''); else moveNote(noteDragSrc, ''); } });

// ---- folder actions ----
async function createFolderAt(parentPath){
  const n = await askName('ชื่อโฟลเดอร์ใหม่', '');
  if (!n) return;
  const rel = parentPath ? parentPath + '/' + n.trim() : n.trim();
  const r = await window.api.folderCreate(rel);
  if (r && r.error === 'exists') { alert('มีโฟลเดอร์ชื่อนี้อยู่แล้ว'); return; }
...
function openFolderMenu(anchor, node){
  ...
  const items = [
    ['โน้ตใหม่ในโฟลเดอร์นี้', async () => { const nm = await askName('ตั้งชื่อโน้ตใหม่', ''); if (!nm) return; const r = await window.api.createNote(node.path + '/' + nm.trim()); if (r && r.error === 'exists') { alert('มีโน้ตชื่อนี้อยู่แล้ว'); return; } collapsedFolders.delete(node.path); await refreshList(r && r.name); }],
    ['โฟลเดอร์ย่อยใหม่', () => createFolderAt(node.path)],
    ['เปลี่ยนชื่อโฟลเดอร์', async () => { const nm = await askName('เปลี่ยนชื่อโฟลเดอร์', node.name); if (!nm || nm.trim() === node.name) return; const parent = node.path.includes('/') ? node.path.slice(0, node.path.lastIndexOf('/')) : ''; const to = parent ? parent + '/' + nm.trim() : nm.trim(); const r = await window.api.folderRename(node.path, to); if (r && r.error) { alert('เปลี่ยนชื่อไม่ได้'); return; } await refreshList(currentNote); }],
    ['ลบโฟลเดอร์ (ไปถังขยะ)', async () => { if (!confirm('ย้ายโฟลเดอร์ "' + node.name + '" และทุกอย่างข้างในไปถังขยะ?')) return; await window.api.folderDelete(node.path); await refreshList(); }],
  ];
  items.forEach(([label, fn]) => { const it = document.createElement('div'); it.className = 'db-mi' + (label.startsWith('ลบ') ? ' db-mi-del' : ''); it.textContent = label; it.onclick = () => { closeFolderMenu(); fn(); }; menu.appendChild(it); });
  document.body.appendChild(menu);
...
function openBrokenLinkMenu(target, el){
  ...
  const hd = document.createElement('div'); hd.className = 'lp-hd'; hd.textContent = '“' + target + '” ยังไม่มีโน้ต'; menu.appendChild(hd);
  const sub = document.createElement('div'); sub.className = 'lp-sub'; sub.textContent = 'สร้างหน้าเนื้อหาสำหรับลิงก์นี้'; menu.appendChild(sub);

  const blank = document.createElement('div'); blank.className = 'db-mi lp-add'; blank.textContent = '＋ สร้างโน้ตเปล่า';
  blank.onclick = async () => {
    closeBrokenMenu();
    ...
    if (r && r.error && r.error !== 'exists') { alert('สร้างโน้ตไม่ได้'); return; }
    await refreshList(keep || (r && r.name) || (target + '.md'));   // stay on current note; link now renders valid
  };
  menu.appendChild(blank);

  const ai = document.createElement('div'); ai.className = 'db-mi lp-item'; ai.innerHTML = icoSvg('sparkle', 'sm'); ai.appendChild(document.createTextNode(' ให้ AI เขียนเนื้อหาให้'));
  ai.onclick = async () => {
    closeBrokenMenu();
...
async function openNote(name) {
  ...
  try { localStorage.setItem('lastOpen', JSON.stringify({ type: 'note', name })); } catch (_) {}
...
document.getElementById('newNoteBtn').onclick = async () => {
  const n = await askName('ตั้งชื่อโน้ตใหม่', '');
  if (!n) return;
  const r = await window.api.createNote(n);
  if (r.error === 'exists') { alert('มีโน้ตชื่อนี้อยู่แล้ว'); return; }
...
function openNoteMenu(x, y, rel){
  ...
  const menu = document.createElement('div'); menu.className = 'db-menu'; menu.id = 'folderMenu';
  [['เปลี่ยนชื่อ', () => renameNoteAt(rel)], ['ลบ (ไปถังขยะ)', () => deleteNoteAt(rel)]].forEach(([label, fn]) => {
    const it = document.createElement('div'); it.className = 'db-mi' + (label.startsWith('ลบ') ? ' db-mi-del' : '');
...
async function refreshBacklinks(name) {
  ...
  label.innerHTML = icoSvg('link', 'sm'); label.appendChild(document.createTextNode(' อ้างถึงโน้ตนี้ (' + links.length + '): '));
...
function mdToHtml(md){
  ...
  for (const line of lines){
    ...
    let m;
    if ((m = /^(#{1,6})\s+(.*)$/.exec(line))){ closeList(); const lvl = m[1].length; html += '<h'+lvl+'>'+_mdInline(m[2])+'</h'+lvl+'>'; continue; }
    if (/^\s*(-{3,}|\*{3,}|_{3,})\s*$/.test(line)){ closeList(); html += '<hr>'; continue; }
    if (/^\s*[-*+]\s+/.test(line)){ if (listType !== 'ul'){ closeList(); html += '<ul>'; listType = 'ul'; } html += '<li>'+_mdInline(line.replace(/^\s*[-*+]\s+/,''))+'</li>'; continue; }
    if (/^\s*\d+\.\s+/.test(line)){ if (listType !== 'ol'){ closeList(); html += '<ol>'; listType = 'ol'; } html += '<li>'+_mdInline(line.replace(/^\s*\d+\.\s+/,''))+'</li>'; continue; }
    if (/^\s*>\s?/.test(line)){ closeList(); html += '<blockquote>'+_mdInline(line.replace(/^\s*>\s?/,''))+'</blockquote>'; continue; }
...
function openDiffReview(name, before, after){
  ...
  segs.forEach((seg) => {
    ...
    const y = document.createElement('b'); y.className = 'y'; y.innerHTML = icoSvg('check', 'xs'); y.title = 'รับจุดนี้';
    const e = document.createElement('b'); e.className = 'e'; e.innerHTML = icoSvg('pencil', 'xs'); e.title = 'แก้ก่อนรับ';
    const n = document.createElement('b'); n.className = 'n'; n.innerHTML = icoSvg('x', 'xs'); n.title = 'ทิ้งจุดนี้';
    ...
    const renderFlow = () => {
      ...
      if (state.accept){
        ...
        const ph = document.createElement('div'); ph.className = 'sg-rich sg-ph'; ph.textContent = '(ทิ้งการเพิ่มนี้)';
        flow.appendChild(ph);
      }
    };
    const applyState = () => {
  ...
  info.innerHTML = 'AI เสนอแก้ <b>' + hunkSegs.length + '</b> จุด — เลือกรับ/แก้/ทิ้งที่แต่ละจุด';
  const sp = document.createElement('span'); sp.className = 'sg-sp';
  const noneBtn = document.createElement('button'); noneBtn.className = 'sg-btn none'; noneBtn.textContent = 'ทิ้งทั้งหมด';
  const allBtn = document.createElement('button'); allBtn.className = 'sg-btn all'; allBtn.textContent = 'รับทั้งหมด';
  const useBtn = document.createElement('button'); useBtn.className = 'sg-btn use'; useBtn.textContent = 'เสร็จ · ใช้ที่เลือก';
...
function openQuickAsk() {
  ...
  card.className = 'modal-card';
  const h = document.createElement('h3');
  h.textContent = 'ถาม AI เกี่ยวกับโน้ตนี้';
...
function rowTitle(db, row){ const c = dbTitleCol(db); const v = c ? row[c.id] : ''; return (v == null || v === '') ? '(ไม่มีชื่อ)' : String(v); }
...
async function renderDbList(host){
  ...
  const title = document.createElement('span'); title.className = 'vh-title'; title.textContent = 'ฐานข้อมูล';
  ...
  dbs.forEach((d) => {
    ...
    const meta = document.createElement('div'); meta.className = 'meta'; meta.textContent = d.rows + ' แถว · ' + d.cols + ' คอลัมน์';
  ...
  const lab = document.createElement('div'); lab.textContent = 'สร้างฐานข้อมูลใหม่';
  add.appendChild(plus); add.appendChild(lab);
  add.onclick = async () => { const db = await window.api.dbCreate({}); if (db) { dbOpenId = db.id; renderTable(); } };
...
async function renderDbTable(host){
  ...
  host.innerHTML = '';
  const head = document.createElement('div'); head.className = 'view-head';
  const back = document.createElement('button'); back.className = 'ghost sm'; back.textContent = '‹ ฐานข้อมูล'; back.onclick = () => { dbOpenId = null; dbCache = null; renderTable(); };
  ...
  DB_VIEWS.forEach(([v, ic, label]) => {
    const b = document.createElement('button'); b.className = 'db-vbtn' + (db.view === v ? ' on' : ''); b.innerHTML = icoSvg(ic, 'sm'); b.appendChild(document.createTextNode(' ' + label)); b.title = label;
  ...
  const addRow = document.createElement('button'); addRow.className = 'solid sm'; addRow.textContent = '＋ แถว'; addRow.onclick = () => addDbRow();
...
function renderTableBody(body, db){
  ...
  db.columns.forEach((col, i) => {
    ...
    const grip = document.createElement('div'); grip.className = 'db-res'; grip.title = 'ลากเพื่อปรับความกว้าง';
  ...
  const addTh = document.createElement('th'); addTh.className = 'db-addcol'; addTh.textContent = '＋'; addTh.title = 'เพิ่มคอลัมน์';
  ...
  const addRowBar = document.createElement('div'); addRowBar.className = 'db-addrow'; addRowBar.textContent = '＋ เพิ่มแถว'; addRowBar.onclick = () => addDbRow();
...
function renderBoardView(body, db){
  ...
  if (!selCols.length){ dbEmptyHint(body, 'บอร์ดต้องมีคอลัมน์แบบ “เลือก” (select) อย่างน้อย 1 คอลัมน์ — เพิ่มในมุมมองตาราง'); return; }
  ...
  bar.appendChild(pickColControl('จัดกลุ่มตาม', selCols, db.boardBy, (v) => { db.boardBy = v; saveDb(); renderTable(); }));
  ...
  groups.forEach((g) => {
    ...
    const pill = document.createElement('span'); pill.className = 'db-pill'; const c = DB_COLORS[g.color || 'gray']; pill.style.background = c[0]; pill.style.color = c[1]; pill.textContent = g.empty ? 'ไม่ระบุ' : g.key;
    ...
    const add = document.createElement('div'); add.className = 'db-lane-add'; add.textContent = '＋ การ์ด';
...
function renderCalendarView(body, db){
  ...
  if (!dateCols.length){ dbEmptyHint(body, 'ปฏิทินต้องมีคอลัมน์แบบ “วันที่” (date) — เพิ่มในมุมมองตาราง'); return; }
  ...
  const lbl = document.createElement('span'); lbl.className = 'db-cal-title'; lbl.textContent = THM[calCursor.m] + ' ' + (calCursor.y + 543);
  const today = document.createElement('button'); today.className = 'ghost sm'; today.textContent = 'วันนี้'; today.onclick = () => { calCursor = { y: now.getFullYear(), m: now.getMonth() }; renderTable(); };
  bar.appendChild(prev); bar.appendChild(lbl); bar.appendChild(next); bar.appendChild(today);
  bar.appendChild(pickColControl('วันที่จาก', dateCols, db.calBy, (v) => { db.calBy = v; saveDb(); renderTable(); }));
  body.appendChild(bar);

  const grid = document.createElement('div'); grid.className = 'db-cal';
  ['อา','จ','อ','พ','พฤ','ศ','ส'].forEach((d) => { const h = document.createElement('div'); h.className = 'db-cal-wd'; h.textContent = d; grid.appendChild(h); });
...
function renderGalleryView(body, db){
  ...
  const add = document.createElement('div'); add.className = 'db-gcard add'; add.textContent = '＋ เพิ่มแถว'; add.onclick = () => addDbRow();
  grid.appendChild(add);
...
function renderChartView(body, db){
  ...
  if (!selCols.length){ dbEmptyHint(body, 'ชาร์ตต้องมีคอลัมน์แบบ “เลือก” (select) สำหรับจัดกลุ่ม'); return; }
  ...
  const bar = document.createElement('div'); bar.className = 'db-viewbar';
  bar.appendChild(pickColControl('จัดกลุ่มตาม', selCols, db.chartBy, (v) => { db.chartBy = v; saveDb(); renderTable(); }));
  // aggregation control
  const aggWrap = document.createElement('div'); aggWrap.className = 'db-pickctl';
  const al = document.createElement('span'); al.className = 'db-pickctl-l'; al.textContent = 'ค่า'; aggWrap.appendChild(al);
  const aggSel = document.createElement('select');
  const oc = document.createElement('option'); oc.value = 'count'; oc.textContent = 'จำนวนแถว'; aggSel.appendChild(oc);
  numCols.forEach((c) => { const o = document.createElement('option'); o.value = c.id; o.textContent = 'ผลรวม ' + c.name; if (c.id === db.chartAgg) o.selected = true; aggSel.appendChild(o); });
  ...
  data.forEach((d) => {
    ...
    const lab = document.createElement('span'); lab.className = 'db-chart-lab'; lab.textContent = d.label; row.appendChild(lab);
...
function openRowEditor(db, row){
  ...
  const del = document.createElement('button'); del.className = 'ghost'; del.textContent = 'ลบแถว'; del.onclick = () => { dbCache.rows = dbCache.rows.filter((r) => r.id !== row.id); saveDb(); back.remove(); renderTable(); };
  const done = document.createElement('button'); done.className = 'solid'; done.textContent = 'เสร็จ'; done.onclick = () => { back.remove(); renderTable(); };
...
function buildDbRow(db, row){
  ...
  const del = document.createElement('td'); del.className = 'db-delrow'; del.innerHTML = icoSvg('x', 'xs'); del.title = 'ลบแถว';
  del.onclick = () => { dbCache.rows = dbCache.rows.filter((r) => r.id !== row.id); saveDb(); renderTable(); };
  tr.appendChild(del);
  return tr;
}
...
function fillCell(el, col, row, onChange, isTd){
  ...
  if (col.type === 'checkbox'){
    ...
    else { const ph = document.createElement('span'); ph.className = 'db-empty'; ph.textContent = '＋ เชื่อมโยง'; el.appendChild(ph); }
...
function openRelationPicker(anchor, col, row, rer){
  ...
  if (!target){ const m = document.createElement('div'); m.className = 'db-mi db-mi-mut'; m.textContent = 'DB ปลายทางหาย'; menu.appendChild(m); placeDbMenu(menu, anchor); return; }
  const hd = document.createElement('div'); hd.className = 'lp-sub'; hd.textContent = 'เชื่อมกับ “' + target.name + '”'; menu.appendChild(hd);
  const search = document.createElement('input'); search.className = 'db-hinput'; search.placeholder = 'ค้นหา…'; menu.appendChild(search);
  ...
  const draw = (q) => {
    ...
    if (!listWrap.children.length){ const e = document.createElement('div'); e.className = 'db-mi db-mi-mut'; e.textContent = 'ไม่พบแถว'; listWrap.appendChild(e); }
  };
...
function openSelectMenu(anchor, col, row, rer){
  ...
  const clear = document.createElement('div'); clear.className = 'db-mi db-mi-mut'; clear.textContent = 'ล้างค่า';
  clear.onclick = () => { row[col.id] = ''; done(); };
  menu.appendChild(clear);
  const addWrap = document.createElement('div'); addWrap.className = 'db-addopt';
  const inp = document.createElement('input'); inp.className = 'db-hinput'; inp.placeholder = '＋ ตัวเลือกใหม่ (Enter)';
...
function openColMenu(th, col){
  closeDbMenu();
  const menu = document.createElement('div'); menu.className = 'db-menu'; menu.id = 'dbMenu';
  const ren = document.createElement('div'); ren.className = 'db-mi'; ren.textContent = 'เปลี่ยนชื่อคอลัมน์'; ren.onclick = () => { closeDbMenu(); renameColHeader(th, col); };
  menu.appendChild(ren);
  if (dbCache.columns.length > 1) {
    const del = document.createElement('div'); del.className = 'db-mi db-mi-del'; del.textContent = 'ลบคอลัมน์';
    del.onclick = async () => {
      closeDbMenu();
...
function addColumn(type, extra){
  const col = Object.assign({ id: dbNewId('c'), name: 'คอลัมน์ใหม่', type }, extra || {});
...
async function openRelTargetMenu(anchor){
  ...
  const dbs = await window.api.dbList();
  const menu = document.createElement('div'); menu.className = 'db-menu'; menu.id = 'dbMenu';
  const hd = document.createElement('div'); hd.className = 'lp-sub'; hd.textContent = 'เชื่อมโยงไปยังฐานข้อมูล:'; menu.appendChild(hd);
  dbs.forEach((d) => {
    const it = document.createElement('div'); it.className = 'db-mi';
  ...
  if (!dbs.length){ const e = document.createElement('div'); e.className = 'db-mi db-mi-mut'; e.textContent = 'ยังไม่มีฐานข้อมูลอื่น'; menu.appendChild(e); }
  placeDbMenu(menu, anchor);
}
...
function openDbMenu(anchor){
  closeDbMenu();
  const menu = document.createElement('div'); menu.className = 'db-menu'; menu.id = 'dbMenu';
  const ren = document.createElement('div'); ren.className = 'db-mi'; ren.textContent = 'เปลี่ยนชื่อฐานข้อมูล'; ren.onclick = () => { closeDbMenu(); renameDbTitle(); };
  const del = document.createElement('div'); del.className = 'db-mi db-mi-del'; del.textContent = 'ลบฐานข้อมูล';
  del.onclick = async () => { closeDbMenu(); await window.api.dbDelete(dbOpenId); dbOpenId = null; dbCache = null; renderTable(); };
...
function defaultTaskDb(){ return localStorage.getItem('taskDbId') || ''; }
function setDefaultTaskDb(id){ localStorage.setItem('taskDbId', id || ''); }
async function openAddToDbMenu(text, anchor){
  ...
  const hd = document.createElement('div'); hd.className = 'lp-sub'; hd.textContent = 'เพิ่ม “' + label + '” เป็นรายการใน:'; menu.appendChild(hd);
  dbs.forEach((d) => {
    ...
    it.appendChild(nm);
    if (d.id === defId){ const bg = document.createElement('span'); bg.className = 'dt-badge'; bg.textContent = 'ค่าเริ่มต้น'; it.appendChild(bg); }
    const star = document.createElement('span'); star.className = 'dt-star' + (d.id === defId ? ' on' : ''); star.innerHTML = icoSvg('star', 'sm');
    star.title = d.id === defId ? 'เลิกผูกเป็น DB เริ่มต้น' : 'ผูกเป็น DB เริ่มต้นสำหรับ Task';
  ...
  const add = document.createElement('div'); add.className = 'db-mi lp-add'; add.textContent = 'สร้าง DB “งานประจำวัน” ใหม่';
...
function toastAddedToDb(db){
  ...
  const msg = document.createElement('span'); msg.textContent = 'เพิ่มลง “' + db.name + '” แล้ว';
  const open = document.createElement('button'); open.className = 'toast-btn'; open.textContent = 'เปิดตาราง';
  open.onclick = () => { dbOpenId = db.id; dbCache = null; if (typeof setMainView === 'function') setMainView('table'); t.remove(); };
  t.appendChild(msg); t.appendChild(open); document.body.appendChild(t);
...
async function renderGraph(){
  ...
  host.innerHTML = '';
  const head = document.createElement('div'); head.className = 'view-head';
  const title = document.createElement('span'); title.className = 'vh-title'; title.textContent = 'บอร์ดความเชื่อมโยง (' + data.nodes.length + ' โน้ต, ' + data.edges.length + ' ลิงก์)';
  const back = document.createElement('button'); back.className = 'ghost sm'; back.textContent = '↩ โน้ต'; back.onclick = () => setMainView('note');
  ...
  if (!data.nodes.length) { const e = document.createElement('div'); e.className = 'cork-empty'; e.textContent = 'ยังไม่มีลิงก์ระหว่างโน้ต — เพิ่ม [[ชื่อโน้ต]] ในเนื้อหาเพื่อสร้างความเชื่อมโยง'; stage.appendChild(e); return; }
...
function applyTheme(t) {
  ...
  { const _tb = document.getElementById('themeBtn2'); if (_tb) { const u = _tb.querySelector('use'); if (u) u.setAttribute('href', t === 'dark' ? '#i-sun' : '#i-moon'); } }
  applyTermTheme();
}
...
function openSettings(){
  ...
  const title=document.createElement('span'); title.textContent='ตั้งค่า';
  ...
  const info=document.createElement('p'); info.className='settings-info'; info.textContent='แก้ข้อความคำสั่ง AI ได้ ใช้ {term} = คำที่เลือก, {file} = ชื่อไฟล์โน้ต'; card.appendChild(info);
  const fields={};
  Object.keys(DEFAULT_PROMPTS).forEach(key=>{
    const wrap=document.createElement('div'); wrap.className='settings-field';
    const lab=document.createElement('label'); lab.textContent=PROMPT_LABELS[key]||key;
  ...
  const reset=document.createElement('button'); reset.className='ghost'; reset.textContent='คืนค่าเริ่มต้น';
  const save=document.createElement('button'); save.className='solid'; save.textContent='บันทึก';
...
function applyRightMode(){
  ...
  { const _cm = document.getElementById('chatModeBtn'); if (_cm) _cm.innerHTML = icoSvg(rightMode === 'chat' ? 'terminal' : 'chat'); }
  if (rightMode === 'term') setTimeout(fitTerm, 30);
}
applyRightMode();
document.getElementById('chatModeBtn').onclick = () => { rightMode = rightMode === 'chat' ? 'term' : 'chat'; localStorage.setItem('rightMode', rightMode); applyRightMode(); };
loadSessions();
...
(async () => {
  let lastOpen = null;
  try { lastOpen = JSON.parse(localStorage.getItem('lastOpen') || 'null'); } catch (_) {}
  const lastNote = localStorage.getItem('lastNote') || 'ระบบไต.md';
...
async function runAutoLink(){
  ...
  if (!names.length) { const bar = autolinkBarEl(); if (bar) { bar.innerHTML = '<div class="al-empty">ยังไม่มีโน้ตอื่นให้ลิงก์</div>'; bar.hidden = false; setTimeout(clearAutolink, 2400); } return; }
  autolinkNameSet = new Set(names.map((n) => n.toLowerCase()));
  autolinkAcc = ''; autolinkRunning = true;
  const btn = document.getElementById('autolinkBtn'); if (btn) { btn.disabled = true; btn.innerHTML = icoSvg('refresh'); btn.title = 'กำลังหา…'; }
  const bar = autolinkBarEl(); if (bar) { bar.innerHTML = '<div class="al-empty">AI กำลังหาจุดที่ลิงก์ได้…</div>'; bar.hidden = false; }
  const prompt =
...
function finishAutoLink(){
  ...
  const btn = document.getElementById('autolinkBtn'); if (btn) { btn.disabled = false; btn.innerHTML = icoSvg('thread'); btn.title = 'ร้อยเชือก — AI แนะนำลิงก์ไปโน้ตอื่น'; }
...
function renderAutolinkBar(list){
  ...
  bar.innerHTML = '';
  if (!list.length){ bar.innerHTML = '<div class="al-empty">ไม่พบจุดที่ควรลิงก์เพิ่ม</div>'; bar.hidden = false; setTimeout(clearAutolink, 2600); return; }
  const hd = document.createElement('div'); hd.className = 'al-hd';
  hd.innerHTML = 'พบ <b>' + list.length + '</b> จุดที่ลิงก์ได้ · กดเครื่องหมายถูกเพื่อแทรก';
  const x = document.createElement('button'); x.className = 'al-x'; x.innerHTML = icoSvg('x','xs'); x.title = 'ปิด'; x.onclick = clearAutolink;
  ...
  const all = document.createElement('button'); all.className = 'al-all'; all.innerHTML = icoSvg('check','xs'); all.appendChild(document.createTextNode(' แทรกทั้งหมด'));
...
async function openLinkPicker(term, anchor){
  ...
  const hd = document.createElement('div'); hd.className = 'lp-hd'; hd.textContent = 'ลิงก์ “' + term + '” ไปยัง:'; menu.appendChild(hd);
  if (scored.length){
    const sub = document.createElement('div'); sub.className = 'lp-sub'; sub.textContent = exact ? 'มีโน้ตนี้อยู่แล้ว — ลิงก์ไปเลย (ไม่ต้องสร้างซ้ำ)' : 'โน้ตเดิมที่ใกล้เคียง — ลิงก์ไปอันเดิมไหม?';
    ...
    scored.forEach((x) => {
      ...
      const bd = document.createElement('span'); bd.className = 'lp-badge' + (x.s >= 0.999 ? ' exact' : ''); bd.textContent = x.s >= 0.999 ? 'ตรงกัน' : ('ใกล้เคียง ' + Math.round(x.s * 100) + '%');
      it.appendChild(nm); it.appendChild(bd);
      it.onclick = () => { closeLinkPicker(); insertOneLink(term, x.name, false); };
      menu.appendChild(it);
    });
  } else {
    const none = document.createElement('div'); none.className = 'lp-sub'; none.textContent = 'ไม่พบโน้ตเดิมที่ใกล้เคียง';
    menu.appendChild(none);
  }
  const add = document.createElement('div'); add.className = 'db-mi lp-add'; add.textContent = '＋ สร้างโน้ตใหม่ “' + term + '” แล้วลิงก์';
...
async function insertOneLink(term, targetName, createNew){
  if (!currentNote) return;
  if (createNew){ const r = await window.api.createNote(targetName); if (r && r.error && r.error !== 'exists') { alert('สร้างโน้ตไม่ได้'); return; } }
  let body = getBody();
...
function openSideChat(seed){
  ...
  const title = document.createElement('span'); title.className = 'sc-title'; title.innerHTML = icoSvg('chat','sm'); title.appendChild(document.createTextNode(' แชตลอย'));
  ...
  const ctx = document.createElement('button'); ctx.className = 'sc-ctx' + (sc.scope === 'note' ? ' on' : ''); ctx.innerHTML = icoSvg('clip', 'sm'); ctx.title = 'อ้างอิงโน้ตที่เปิดอยู่';
  ...
  const x = document.createElement('button'); x.className = 'sc-x'; x.innerHTML = icoSvg('x','sm'); x.title = 'ปิด';
  ...
  const ta = document.createElement('textarea'); ta.className = 'sc-input'; ta.rows = 1; ta.placeholder = 'ถามเร็ว ๆ… (Enter ส่ง)';
  const send = document.createElement('button'); send.className = 'sc-send'; send.innerHTML = icoSvg('send','sm'); send.title = 'ส่ง';
...
function renderSideMsgs(sc){
  ...
  if (!sc.messages.length){ const h = document.createElement('div'); h.className = 'sc-hint'; h.innerHTML = icoSvg('clip', 'sm'); h.appendChild(document.createTextNode(sc.scope === 'note' ? ' ถามเกี่ยวกับโน้ตนี้ได้เลย' : ' ถามอะไรก็ได้ (แชตนี้แยกจากแท็บหลัก)')); box.appendChild(h); }
...
window.api.onEngineDone((p) => {
  ...
  if (last && last.role === 'ai'){ last.text = cleanChatText(last._acc || '').trim() || '(เสร็จ · ไม่มีข้อความตอบกลับ)'; delete last._acc; }
...
function loadDashboards(){ try { const d = JSON.parse(localStorage.getItem('dashboards') || 'null'); return Array.isArray(d) ? d : null; } catch (_) { return null; } }
function saveDashboards(d){ try { localStorage.setItem('dashboards', JSON.stringify(d)); } catch (_) {} }
function ensureDashboards(){
  let d = loadDashboards();
  if (!d || !d.length){
    let old = []; try { old = JSON.parse(localStorage.getItem('dashWidgets') || '[]'); } catch (_) {}
    d = [{ id: 'dash1', name: 'แดชบอร์ด 1', widgets: Array.isArray(old) ? old : [] }];
    saveDashboards(d);
  }
  return d;
}
function dashSeq(){ let n = parseInt(localStorage.getItem('dashSeq') || '1', 10) || 1; n++; try { localStorage.setItem('dashSeq', String(n)); } catch (_) {} return n; }
function currentDashId(){ const d = ensureDashboards(); const id = localStorage.getItem('curDashId'); return (id && d.some((x) => x.id === id)) ? id : d[0].id; }
function setCurrentDashId(id){ try { localStorage.setItem('curDashId', id); } catch (_) {} }
...
async function renderDash(){
  ...
  const head = document.createElement('div'); head.className = 'view-head';
  const title = document.createElement('span'); title.className = 'vh-title'; title.textContent = (typeof currentDashboard === 'function' ? currentDashboard().name : 'แดชบอร์ด');
  const sp = document.createElement('span'); sp.className = 'vh-sp';
  const add = document.createElement('button'); add.className = 'solid sm'; add.textContent = '＋ เพิ่มการ์ด';
  ...
  if (!widgets.length){
    ...
    empty.innerHTML = '<b>ยังไม่มีการ์ด</b><br>กด “＋ เพิ่มการ์ด” เพื่อเลือกฐานข้อมูลและรูปแบบการแสดงผล';
...
function buildDashCard(w, idx, db, widgets){
  ...
  const t = document.createElement('span'); t.className = 'dash-t';
    if (db){ t.innerHTML = icoSvg('db', 'xs'); t.appendChild(document.createTextNode(' ' + (w.title || db.name))); }
    else { t.textContent = 'DB หาย'; }
  t.title = 'เปิดฐานข้อมูลนี้';
  t.onclick = () => { if (!db) return; dbOpenId = db.id; dbCache = null; setMainView('table'); };
    const kind = document.createElement('span'); kind.className = 'dash-kind'; kind.innerHTML = icoSvg(DASH_VIZ_ICON[w.viz] || 'table', 'xs'); kind.appendChild(document.createTextNode(' ' + (DASH_VIZ_LABEL[w.viz] || w.viz)));
  ...
  h.appendChild(t); h.appendChild(kind); h.appendChild(dots); card.appendChild(h);
  const body = document.createElement('div'); body.className = 'dash-body'; card.appendChild(body);
  if (!db){ body.innerHTML = '<div class="dash-warn">ฐานข้อมูลถูกลบไปแล้ว</div>'; return card; }
  try { renderDashViz(body, db, w); } catch (_) { body.innerHTML = '<div class="dash-warn">แสดงผลไม่ได้</div>'; }
  return card;
}
...
function renderDashViz(body, db, w){
  ...
  if (w.viz === 'stat'){
    ...
    const lab = document.createElement('div'); lab.className = 'ds-lab'; lab.textContent = 'รายการทั้งหมด';
    ...
    if (g){
      ...
      g.groups.forEach((gr) => {
        ...
        p.style.background = c[0]; p.style.color = c[1]; p.textContent = (gr.empty ? 'ไม่ระบุ' : gr.key) + ' · ' + gr.rows.length;
        pills.appendChild(p);
      });
  ...
  if (w.viz === 'chart'){
    const g = dashGroups(db, w.groupBy);
    if (!g){ body.innerHTML = '<div class="dash-warn">ต้องมีคอลัมน์แบบ “เลือก”</div>'; return; }
    ...
    g.groups.forEach((gr) => {
      ...
      const lab = document.createElement('span'); lab.className = 'db-chart-lab'; lab.textContent = gr.empty ? 'ไม่ระบุ' : gr.key;
      const track = document.createElement('div'); track.className = 'db-chart-track';
  ...
  if (w.viz === 'board'){
    ...
    g.groups.forEach((gr) => {
      ...
      const p = document.createElement('span'); p.className = 'db-pill sm'; const c = DB_COLORS[gr.color || 'gray'];
      p.style.background = c[0]; p.style.color = c[1]; p.textContent = gr.empty ? 'ไม่ระบุ' : gr.key;
      ...
      if (gr.rows.length > 4){ const m = document.createElement('div'); m.className = 'dash-more'; m.textContent = '+' + (gr.rows.length - 4) + ' รายการ'; lane.appendChild(m); }
  ...
  if (w.viz === 'table'){
    ...
    if (rows.length > 8){ const m = document.createElement('div'); m.className = 'dash-more'; m.textContent = '+' + (rows.length - 8) + ' แถว'; body.appendChild(m); }
    return;
  }
  if (w.viz === 'list'){
    ...
    body.appendChild(ul);
    if (rows.length > 12){ const m = document.createElement('div'); m.className = 'dash-more'; m.textContent = '+' + (rows.length - 12) + ' รายการ'; body.appendChild(m); }
    return;
  }
  if (w.viz === 'calendar'){
    const dcol = firstColOfType(db, 'date');
    if (!dcol){ body.innerHTML = '<div class="dash-warn">ต้องมีคอลัมน์แบบ “วันที่”</div>'; return; }
    ...
    cap.textContent = ['มกราคม','กุมภาพันธ์','มีนาคม','เมษายน','พฤษภาคม','มิถุนายน','กรกฎาคม','สิงหาคม','กันยายน','ตุลาคม','พฤศจิกายน','ธันวาคม'][m] + ' ' + (y + 543);
    wrap.appendChild(cap);
    const grid = document.createElement('div'); grid.className = 'dash-cal';
    ['อา','จ','อ','พ','พฤ','ศ','ส'].forEach((d) => { const e = document.createElement('div'); e.className = 'dash-cal-wd'; e.textContent = d; grid.appendChild(e); });
    const startDow = new Date(y, m, 1).getDay(), days = new Date(y, m + 1, 0).getDate(), t = todayISO();
    for (let i = 0; i < startDow; i++) grid.appendChild(document.createElement('div'));
    for (let d = 1; d <= days; d++){
      const iso = y + '-' + pad2(m + 1) + '-' + pad2(d);
      const cell = document.createElement('div'); cell.className = 'dash-cal-d' + (iso === t ? ' today' : '');
      cell.textContent = d;
      const n = rows.filter((r) => (r[dcol.id] || '') === iso).length;
      if (n){ cell.classList.add('has'); cell.title = n + ' รายการ'; const dot = document.createElement('i'); cell.appendChild(dot); }
      grid.appendChild(cell);
    }
    wrap.appendChild(grid); body.appendChild(wrap); return;
  }
  body.innerHTML = '<div class="dash-warn">ไม่รู้จักรูปแบบนี้</div>';
}
...
function openDashAddMenu(anchor, dbs){
  closeDbMenu();
  const menu = document.createElement('div'); menu.className = 'db-menu'; menu.id = 'dbMenu';
  const hd = document.createElement('div'); hd.className = 'lp-sub'; hd.textContent = 'เลือกฐานข้อมูล:'; menu.appendChild(hd);
  if (!dbs.length){ const e = document.createElement('div'); e.className = 'db-mi db-mi-mut'; e.textContent = 'ยังไม่มีฐานข้อมูล'; menu.appendChild(e); }
  dbs.forEach((d) => {
...
function openDashVizMenu(anchor, d){
  closeDbMenu();
  const menu = document.createElement('div'); menu.className = 'db-menu'; menu.id = 'dbMenu';
  const hd = document.createElement('div'); hd.className = 'lp-sub'; hd.textContent = 'แสดง “' + d.name + '” แบบ:'; menu.appendChild(hd);
  DASH_VIZ.forEach(([viz, ic, label]) => {
    const it = document.createElement('div'); it.className = 'db-mi'; it.innerHTML = icoSvg(ic, 'sm'); it.appendChild(document.createTextNode(' ' + label));
    it.onclick = () => {
      closeDbMenu();
...
function openDashCardMenu(anchor, idx, widgets){
  ...
  const menu = document.createElement('div'); menu.className = 'db-menu'; menu.id = 'dbMenu';
  const w = widgets[idx];
  const wide = document.createElement('div'); wide.className = 'db-mi'; wide.textContent = (w.span === 2 ? '◧ ย่อความกว้าง' : '◨ ขยายกว้าง 2 ช่อง');
  wide.onclick = () => { const ws = loadDashWidgets(); ws[idx].span = ws[idx].span === 2 ? 1 : 2; saveDashWidgets(ws); closeDbMenu(); renderDash(); };
  const tall = document.createElement('div'); tall.className = 'db-mi'; tall.textContent = (w.tall ? '▭ ลดความสูง' : '▯ เพิ่มความสูง');
  tall.onclick = () => { const ws = loadDashWidgets(); ws[idx].tall = !ws[idx].tall; saveDashWidgets(ws); closeDbMenu(); renderDash(); };
  const up = document.createElement('div'); up.className = 'db-mi'; up.textContent = 'เลื่อนขึ้น';
  up.onclick = () => { const ws = loadDashWidgets(); if (idx > 0){ const t = ws[idx-1]; ws[idx-1] = ws[idx]; ws[idx] = t; saveDashWidgets(ws); } closeDbMenu(); renderDash(); };
  const down = document.createElement('div'); down.className = 'db-mi'; down.textContent = 'เลื่อนลง';
  down.onclick = () => { const ws = loadDashWidgets(); if (idx < ws.length - 1){ const t = ws[idx+1]; ws[idx+1] = ws[idx]; ws[idx] = t; saveDashWidgets(ws); } closeDbMenu(); renderDash(); };
  const del = document.createElement('div'); del.className = 'db-mi db-mi-del'; del.textContent = 'ลบการ์ด';
...
function decorateCrepeToolbar(tb){
  ...
  CREPE_TOOLBAR_EXTRAS.forEach(([icon, title, fn]) => {
    const b = document.createElement('div'); b.className = 'toolbar-item dk-tbi'; b.title = title; b.innerHTML = icoSvg(icon, 'sm');
...
(function shellWiring(){
  const relay = (fromId, toId) => {
    ...
    if (a && b) a.onclick = (e) => { e.preventDefault(); b.click(); };
  };
  relay('themeBtn2', 'themeBtn');
  relay('settingsBtn2', 'settingsBtn');
  ...
  function openNewMenu(anchor){
    ...
    const menu = document.createElement('div'); menu.className = 'db-menu'; menu.id = 'folderMenu';
    const items = [
      ['โน้ตใหม่', () => { const b = document.getElementById('newNoteBtn'); if (b) b.click(); }],
      ['โฟลเดอร์ใหม่', () => { const b = document.getElementById('newFolderBtn'); if (b) b.click(); }],
      ['ฐานข้อมูลใหม่', () => sbNewDb()],
      ['บอร์ดใหม่', () => sbNewDash()],
      ['นำเข้า PDF', () => importPdfFlow()],
    ];
  ...
  if (aiBtn) aiBtn.onclick = (e) => {
    ...
    const items = [
      ['i-tldr', 'สรุปทั้งโน้ต (TL;DR)', 'tldrBtn'],
      ['i-quiz', 'ตั้งคำถามทดสอบ', 'quizBtn'],
      ['i-thread', 'ร้อยเชือก — แนะนำลิงก์', 'autolinkBtn'],
      ['i-chat', 'เปิดแชตลอย', 'sideChatBtn'],
    ];
  ...
  function paintShell(){
      ...
      if (!rel){ const b = document.createElement('b'); b.textContent = 'โน้ต'; crumb.appendChild(b); }
        ...
        parts.forEach((p, i) => {
          ...
          if (i === parts.length - 1){ const b = document.createElement('b'); b.textContent = p; crumb.appendChild(b); }
            ...
            ic.innerHTML = '<use href="#i-folder"/>';
            crumb.appendChild(ic);
            const s = document.createElement('span'); s.textContent = p; crumb.appendChild(s);
          }
        });
      }
    }
    if (savedTag){
      ...
      savedTag.title = isDirty ? 'มีการแก้ที่ยังไม่บันทึก — กด ⌘S เพื่อบันทึก' : 'บันทึกแล้ว';
      const _t = savedTag.querySelector('.saved-txt');
      if (_t) _t.textContent = isDirty ? 'ยังไม่บันทึก' : 'บันทึกแล้ว';
    }
  }
...
async function openPdf(name){
  ...
  if (left){ left.classList.remove('view-graph','view-table','view-dash'); left.classList.add('view-pdf'); }
  mainView = 'pdf';
  ...
  try { localStorage.setItem('lastPdf', name); } catch (_) {}
  try { localStorage.setItem('lastOpen', JSON.stringify({ type: 'pdf', name })); } catch (_) {}
  if (name === pdfLoadedName && pdfDoc && pdfPagesEl() && pdfPagesEl().querySelector('.pdf-page')) return;
  const host = pdfPagesEl();
  if (host) host.innerHTML = '<div class="pdf-empty">กำลังเปิด…</div>';
  const buf = await window.api.readPdf(name);
  if (!buf || !window.pdfjsLib){ if (host) host.innerHTML = '<div class="pdf-empty">เปิดไฟล์ไม่ได้</div>'; return; }
  const data = buf instanceof Uint8Array ? buf : new Uint8Array(buf);
  const token = ++pdfRenderToken;
  try { pdfDoc = await window.pdfjsLib.getDocument({ data }).promise; }
  catch (e) { if (host) host.innerHTML = '<div class="pdf-empty">อ่าน PDF ไม่ได้</div>'; return; }
  if (token !== pdfRenderToken) return;
  pdfLoadedName = name;
  pdfAnnots = await loadPdfAnnots(name);
  let savedPage = 1;
  try { savedPage = parseInt(localStorage.getItem('pdfPage:' + name) || '1', 10) || 1; } catch (_) {}
...
async function renderPdf(token){
  ...
  for (let n = 1; n <= pdfDoc.numPages; n++){
    ...
    const ph = document.createElement('div'); ph.className = 'pdf-ph'; ph.textContent = 'หน้า ' + n; wrap.appendChild(ph);
...
function pdfScrollSync(){
  ...
  if (currentPdf) { try { localStorage.setItem('pdfPage:' + currentPdf, String(cur)); } catch (_) {} }
}
...
async function renamePdf(rel){
  ...
  const nm = await askName('เปลี่ยนชื่อ PDF', cur);
  if (!nm || !nm.trim() || nm.trim() === cur) return;
  const dir = rel.includes('/') ? rel.slice(0, rel.lastIndexOf('/')) : '';
  let to = nm.trim(); if (!/\.pdf$/i.test(to)) to += '.pdf';
  to = dir ? dir + '/' + to : to;
  const r = await window.api.renamePdf(rel, to);
  if (r && r.error === 'exists') { alert('มีไฟล์ชื่อนี้อยู่แล้ว'); return; }
  if (r && r.error) { alert('เปลี่ยนชื่อไม่ได้'); return; }
  const newName = (r && r.name) ? r.name : to;
  try { const pg = localStorage.getItem('pdfPage:' + rel); if (pg != null){ localStorage.setItem('pdfPage:' + newName, pg); localStorage.removeItem('pdfPage:' + rel); } } catch (_) {}
  pdfLoadedName = null;
  await refreshList(currentNote);
...
async function deletePdf(rel){
  if (!rel) return;
  if (!confirm('ย้าย "' + rel.split('/').pop() + '" ไปถังขยะ?')) return;
  await window.api.deleteNote(rel);
  try { localStorage.removeItem('pdfPage:' + rel); } catch (_) {}
...
function openPdfMenu(x, y, rel){
  if (typeof closeFolderMenu === 'function') closeFolderMenu();
  const menu = document.createElement('div'); menu.className = 'db-menu'; menu.id = 'folderMenu';
  [['เปลี่ยนชื่อ', () => renamePdf(rel)], ['ลบ (ไปถังขยะ)', () => deletePdf(rel)]].forEach(([label, fn]) => {
    const it = document.createElement('div'); it.className = 'db-mi' + (label.startsWith('ลบ') ? ' db-mi-del' : '');
    it.textContent = label; it.onclick = () => { if (typeof closeFolderMenu === 'function') closeFolderMenu(); fn(); }; menu.appendChild(it);
  });
  document.body.appendChild(menu);
...
function drawPageHighlights(pageNum, hlLayer, cssW, cssH){
  ...
  pdfAnnots.highlights.filter((h) => h.page === pageNum).forEach((h) => {
    ...
    (h.rects || []).forEach((r) => {
      ...
      d.title = h.note ? h.note : 'คลิกเพื่อเปลี่ยนสี/ลบ';
...
function showHlBar(cx, cy, mode, hl){
  ...
  if (mode === 'new'){
    ...
    const nb = document.createElement('button'); nb.className = 'pdf-sw pdf-sw-act'; nb.type = 'button'; nb.title = 'ไฮไลต์ + จดโน้ต'; nb.innerHTML = icoSvg('chat', 'xs');
  ...
  if (mode === 'edit'){
    ...
    const mk = (icon, title, fn) => { const b = document.createElement('button'); b.className = 'pdf-sw pdf-sw-act'; b.type = 'button'; b.title = title; b.innerHTML = icoSvg(icon, 'xs'); b.onclick = (e) => { e.stopPropagation(); fn(); }; bar.appendChild(b); };
    ...
    const del = document.createElement('button'); del.className = 'pdf-sw pdf-sw-del'; del.type = 'button'; del.title = 'ลบ';
...
function openCommentLinkMenu(x, y, linkEl, hl){
  ...
  const items = [
    ...
    ['แก้ไข Link', () => { if (typeof editHlComment === 'function') editHlComment(hl); }],
  ];
  items.forEach(([label, fn]) => {
    const it = document.createElement('div'); it.className = 'db-mi';
...
async function editHlComment(hl){
  if (!hl) return;
  const val = await askName('คอมเมนต์สำหรับไฮไลต์', hl.note || '');
...
function paAction(icon, title, fn){
  const b = document.createElement('button'); b.className = 'pa-act'; b.type = 'button'; b.title = title;
...
function renderAnnotPanel(){
  ...
  head.textContent = 'ไฮไลต์ · ' + hs.length; p.appendChild(head);
  if (!hs.length){
    const e = document.createElement('div'); e.className = 'pa-empty';
    e.textContent = 'ยังไม่มีไฮไลต์ — ลากเลือกข้อความบนหน้า PDF แล้วเลือกสี'; p.appendChild(e); return;
  }
  hs.forEach((h) => {
    const c = PDF_HL[h.color] || PDF_HL.gold;
    ...
    if (h.note){
      ...
      if (h.area || isPlaceholder){
        const ph = document.createElement('div'); ph.className = 'pa-addnote'; ph.textContent = 'เพิ่มโน้ต…';
    ...
    const pg = document.createElement('span'); pg.className = 'pa-pg'; pg.textContent = 'น.' + h.page; meta.appendChild(pg);
...
async function sendHighlightToNote(hl){
  ...
  if (annotPanelOpen()) renderAnnotPanel();
  pdfToast('ส่งเข้าโน้ตคู่หูแล้ว · หน้า ' + hl.page);
}
...
function unloadPage(wrap){
  ...
  wrap.innerHTML = '';
  const ph = document.createElement('div'); ph.className = 'pdf-ph'; ph.textContent = 'หน้า ' + wrap.dataset.page;
  wrap.appendChild(ph);
...
function makeTboxEl(b, cssW, cssH){
  ...
  const head = document.createElement('div'); head.className = 'pdf-tbox-head'; head.title = 'ลากเพื่อย้าย';
  const body = document.createElement('div'); body.className = 'pdf-tbox-body';
  body.contentEditable = 'true'; body.spellcheck = false;
  body.dataset.ph = 'พิมพ์ที่นี่…';
  body.textContent = b.text || '';

  const del = document.createElement('button'); del.type = 'button';
  del.className = 'pdf-tbox-del'; del.title = 'ลบกล่อง';
...
function showCropMenu(wrap, rect){
  ...
  mk('ครอปเข้าโน้ต', () => cropToNote(wrap, rect, page));
  mk('ไฮไลต์พื้นที่', () => areaHighlight(wrap, rect, page));
  const cancel = document.createElement('button'); cancel.type = 'button'; cancel.className = 'pdf-crop-cancel'; cancel.title = 'ยกเลิก';
...
async function cropToNote(wrap, rect, page){
  ...
  if (!uri) { pdfToast('ครอปไม่ได้ — หน้ายังไม่เรนเดอร์'); return; }
  if (!currentPdf) return;
  const name = await ensureCompanionNote(currentPdf);
  let content = '';
  try { content = await window.api.readNote(name); } catch (_) {}
  ...
  await window.api.saveNote(name, content);
  pdfToast('ครอปเข้าโน้ตคู่หูแล้ว · หน้า ' + page);
}
...
async function movePdf(pdfRel, folderPath){
  ...
  if (r && r.error === 'exists') { alert('มีไฟล์ชื่อนี้อยู่ในโฟลเดอร์นั้นแล้ว'); await refreshList(currentNote); return; }
  if (r && r.error) { await refreshList(currentNote); return; }
  try { const pg = localStorage.getItem('pdfPage:' + pdfRel); if (pg != null){ localStorage.setItem('pdfPage:' + to, pg); localStorage.removeItem('pdfPage:' + pdfRel); } } catch (_) {}
...
var sbDbCache = [];
var sbSecCollapsed = new Set((() => { try { return JSON.parse(localStorage.getItem('sbSecCollapsed') || '[]'); } catch (_) { return []; } })());
function persistSbSec(){ try { localStorage.setItem('sbSecCollapsed', JSON.stringify([...sbSecCollapsed])); } catch (_) {} }
...
async function sbNewDash(){
  const nm = await askName('ชื่อบอร์ดใหม่', '');
...
function sbGroup(key, iconName, label, opts){
  ...
  gh.appendChild(tri);
  // leading section icon only for single-action leaf sections (e.g. กราฟ);
  // the collapsible sections read as plain labels, not icon rows.
  if (opts.leaf){
    const ic = document.createElement('span'); ic.className = 'sb-gic sb-gic-' + iconName; ic.innerHTML = icoSvg(iconName, 'sm'); gh.appendChild(ic);
  }
...
function renderSbDbList(body){
  if (!sbDbCache || !sbDbCache.length){
    const e = document.createElement('div'); e.className = 'sb-empty'; e.textContent = 'ยังไม่มีฐานข้อมูล'; body.appendChild(e); return;
  }
  sbDbCache.forEach((d) => {
    const active = (mainView === 'table' && dbOpenId === d.id);
    body.appendChild(sbLeafItem('db', d.name || 'ฐานข้อมูล', active, () => openDbInView(d.id)));
  });
}
...
function openDashItemMenu(x, y, id){
  ...
  const items = [
    ['เปลี่ยนชื่อ', async () => {
      ...
      const nm = await askName('เปลี่ยนชื่อบอร์ด', b ? b.name : '');
      if (nm && nm.trim()){ renameDashboard(id, nm.trim()); renderSidebar(); if (mainView === 'dash' && currentDashId() === id) renderDash(); }
    }],
    ['ลบบอร์ด', () => {
      if (ensureDashboards().length <= 1){ alert('ต้องมีอย่างน้อย 1 บอร์ด'); return; }
...
function renderSidebar(){
  ...
  host.innerHTML = '';
  const n = sbGroup('note', 'note', 'โน้ต', { count: noteTreeRoot ? countTreeNotes(noteTreeRoot) : 0 });
  host.appendChild(n.group);
  if (!n.collapsed && noteTreeRoot) renderFolderNode(noteTreeRoot, n.body, 0);
  const d = sbGroup('db', 'db', 'ฐานข้อมูล', { addLabel: 'ฐานข้อมูลใหม่', onAdd: sbNewDb, count: (sbDbCache || []).length });
  host.appendChild(d.group);
  if (!d.collapsed) renderSbDbList(d.body);
  const b = sbGroup('dash', 'dash', 'แดชบอร์ด', { addLabel: 'บอร์ดใหม่', onAdd: sbNewDash, count: ensureDashboards().length });
  host.appendChild(b.group);
  if (!b.collapsed) renderSbDashList(b.body);
  const g = sbGroup('graph', 'graph', 'กราฟ', { leaf: true, leafView: 'graph', onLeafClick: () => setMainView('graph') });
